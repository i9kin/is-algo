...
Если ответ `YES`, то массив имеет вид \\(a = [x] \\). Чему равно \\(x\\)?.

Понятно, что это `максимальный` элемент в массиве. До этого не сложно прийти. Достаточно рассмотреть последнее удаление &mdash; мы выбрали \\(x\\) и \\(y\\), где \\(y \le x\\). Следовательно `максимальный` элемент массива \\(a\\) невозможно удалить, а так-же \\(y=x\\) или \\(y=x - 1\\).

Из всего вышеперечисленного можно сделать вывод, что при оптимальном процессе удаления, числа, которые удаляются, не уменьшаются. Значит решение заключается в том, чтобы отсортировать массив и сравнить на разность соседние элементы.
